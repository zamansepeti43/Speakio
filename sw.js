...
self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(async c=>{await Promise.all(CORE.map(async url=>{try{const r=await fetch(url,{cache:'reload'});if(r.ok)await c.put(url,r)}catch(_){}}));await self.skipWaiting()})));

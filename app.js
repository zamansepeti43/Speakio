...
const toast=t=>{const e=$('#toast');if(e){e.textContent=t;e.classList.add('show');setTimeout(()=>e.classList.remove('show'),1600)}};
const addXP=x=>{S.xp=Number(S.xp||0)+x;save();toast('+'+x+' XP ✨')};
function daily(){if(!S.daily||S.daily.date!==today()){S.daily={date:today(),done:0,listening:0,speaking:0};save()}}
function nav(){return `<nav class="bottom"><button class="${view==='home'?'active':''}" onclick="Speakio.go('home')">⌂<small>Ana Sayfa</small></button><button class="${['lessons','lesson','result','review'].includes(view)?'active':''}" onclick="Speakio.go('lessons')">◎<small>Dersler</small></button><button class="${view==='speak'?'active':''}" onclick="Speakio.go('speak')">◉<small>Konuş</small></button><button class="${['profile','settings','analytics'].includes(view)?'active':''}" onclick="Speakio.go('profile')">◌<small>Profil</small></button></nav>`}
function top(t,b='home'){return `<header><button aria-label="Geri" onclick="Speakio.go('${b}')">‹</button><b>${esc(t)}</b><span></span></header>`}
function stats(){return `<div class="stats"><div><strong>🔥 ${Number(S.streak||0)}</strong><small>Gün seri</small></div><div><strong>⚡ ${Number(S.xp||0)}</strong><small>XP</small></div><div><strong>🎯 ${Number(S.goal||3)}</strong><small>Günlük hedef</small></div></div>`}
...
function listenAnswer(i){const choices=[...document.querySelectorAll('.listen .choice')].map(x=>x.textContent.trim());if(norm(choices[i])===norm(listenItem?.text)){daily();S.listeningDone++;S.daily.listening=1;addXP(10);save();toast('+10 XP • Dinleme doğru! 🎧')}else toast('Bir kez daha dinle 🎧');listenItem=null;setTimeout(()=>render(),500)}
function speak(){const a=typeof getSpeakioSpeaking==='function'?getSpeakioSpeaking():[],t=a[S.conversations%Math.max(1,a.length)]||{prompt:'Introduce yourself in English.'};return `<section>${top('AI Speak')}<div class="practice"><div class="coach" aria-hidden="true">🤖</div><div class="bubble"><b>Görev</b><br>${esc(t.prompt)}</div></div><button id="mic" class="mic" aria-label="Konuşmayı başlat" onclick="Speakio.listenSpeech()">🎙️</button><p class="center muted">Mikrofona dokun ve İngilizce konuş.</p><div class="card"><b>İpucu</b><p class="muted">Kısa cümle kur, sonra bir ayrıntı ekle.</p></div></section>`}
function listenSpeech(){const R=window.SpeechRecognition||window.webkitSpeechRecognition;if(!R){toast('Bu tarayıcı ses tanımayı desteklemiyor.');return}const r=new R();r.lang='en-US';r.interimResults=false;r.onstart=()=>$('#mic')?.classList.add('listening');r.onend=()=>$('#mic')?.classList.remove('listening');r.onerror=()=>toast('Mikrofon iznini kontrol et.');r.onresult=e=>{S.conversations++;daily();S.daily.speaking=1;save();toast('Duydum: '+e.results[0][0].transcript)};r.start()}
function analytics(){daily();const total=U().length,done=S.completed.length,completion=total?Math.round(done/total*100):0,avg=Number(S.voiceStats?.average||0),mistakes=S.mistakes||[],due=mistakes.filter(x=>!x.dueAt||Date.parse(x.dueAt)<=Date.now()).length,week=Array.isArray(S.week)?S.week.slice(-7):[0,0,0,0,0,0,0],max=Math.max(1,...week);return `<section>${top('Öğrenme İstatistikleri')}<div class="card"><div class="row"><b>📈 A1 ilerlemesi</b><strong>${completion}%</strong></div><div class="progress"><i style="width:${completion}%"></i></div><p class="muted">${done}/${total} ünite tamamlandı</p></div><div class="grid"><div class="tile"><b>🔥 ${Number(S.streak||0)}</b><small>Günlük seri</small></div><div class="tile"><b>⚡ ${Number(S.xp||0)}</b><small>Toplam XP</small></div><div class="tile"><b>🗣️ ${avg}/100</b><small>Konuşma ortalaması</small></div><div class="tile"><b>🎧 ${Number(S.listeningDone||0)}</b><small>Dinleme</small></div></div><div class="card"><div class="row"><b>📅 Son 7 gün</b><span>çalışma</span></div><div class="weekbars">${week.map((v,i)=>`<div class="weekbar"><i style="height:${Math.max(8,Math.round(Number(v||0)/max*100))}%"></i><small>${['P','S','Ç','P','C','C','P'][i]}</small></div>`).join('')}</div><p class="muted">Günlük hedef: ${S.goal} ders • Bugün ${S.daily.done}/${S.goal}</p></div><div class="card"><b>🧠 Tekrar durumu</b><p>${due} soru şimdi tekrar için hazır.</p><small class="muted">Yanlış sorular 10 dakika sonra; doğru tekrarlar 1 → 3 → 7 → 14 → 30 gün aralıklarla ilerler.</small></div><button class="primary full" onclick="Speakio.go('review')">Tekrara başla</button></section>`}
function grammar(){return `<section>${top('Dilbilgisi')}<div class="card"><span class="pill">A1</span><h2>Temel İngilizce</h2><p class="muted">am/is/are • temel sorular • günlük cümleler • zaman ifadeleri</p></div>${U().map(x=>`<div class="profileline"><b>${esc(x.title)}</b><small>${esc(x.grammar||'Temel yapı')}</small></div>`).join('')}</section>`}
...
function boot(){daily();C=window.SpeakioContent?.course||null;if(C){render();return;}const onReady=e=>{C=e.detail||window.SpeakioContent?.course||null;if(C){render();}};const onError=()=>{const root=$('#app');if(root)root.innerHTML='<main><section><div class="card"><b>İçerik yüklenemedi</b><p class="muted">Kurs verisi hazır değil. Lütfen sayfayı yenileyin veya internet bağlantısını kontrol edin.</p></div></section></main>'};if(window.SpeakioContent?.ready && window.SpeakioContent.course){C=window.SpeakioContent.course;render();return;}window.addEventListener('speakio:content-ready',onReady,{once:true});window.addEventListener('speakio:content-error',onError,{once:true});render();setTimeout(()=>{C=window.SpeakioContent?.course||null;if(C){render();}},100);}
window.Speakio={go,render,startLesson,answer,checkText,addWord,checkBuild,next,saveWord:w=>{if(!S.savedWords.includes(w))S.savedWords.push(w);save();toast('Kelime kaydedildi 🔖')},nextWord:()=>{wordIndex++;render()},say,listenAnswer,listenSpeech,toggle:k=>{S[k]=!S[k];save();render()},rename:v=>{S.name=String(v||'Öğrenci').slice(0,40);save();render()},reset:()=>{if(confirm('Tüm ilerleme silinsin mi?')){localStorage.removeItem(KEY);location.reload()}}};
boot();
})();